...
nsDocument::policyEntry nsDocument::parsePolicy(std::string str){
	...
	if (found != std::string::npos){
		...
		retVal.APIName = str.substr(found + 1);
		std::size_t temp;
...
bool 
nsDocument::checkNodeAgainstSelector(nsIContent *root, const std::string & nodeName, const std::vector<std::string> & selectorAttrName, const std::vector<std::string> & selectorAttrValue){
	//if nodeName doesn't match, return false
	char *nn = ToNewUTF8String(root->NodeName());
	if (nodeName != nn){
		free(nn);
		return false;
	}
...
nsDocument::checkAccessAgainstPolicy(nsIDocument::records::record r, nsDocument::policyEntry p){
	...
	if (p.APIName != ""){
		...
		if (p.APIName[0] == '!') {
			...
			if (forbidden.length() == 0){
				std::string accessEleName = r.resource;
				accessEleName = accessEleName.substr(accessEleName.find_last_of('/') + 1);
				accessEleName = accessEleName.substr(0, accessEleName.find('['));
				...
				std::vector<std::string> tokens{ std::istream_iterator < std::string > {iss}, std::istream_iterator < std::string > {} };
				for (auto t : tokens){
					if (r.additionalInfo == t) return false;		//Policy is given the opposite.
				}
			}
	...
	if (p.eleName.size() == 0 && p.rType == nsDocument::selector) {
		...
		for (auto toMatch : m_SelectorMaps[p.rawValue]){
			...
			int par = p.parent;
			while (par > 0){
				toMatch = toMatch.substr(0, toMatch.rfind('/'));
				par--;
			}
...
void
nsDocument::mapSelectorToXPathVectors(nsIContent *root, const std::vector<policyEntry> & pWithSelector, std::string curXPath, int index){
	...
	for (auto p : pWithSelector){
		if (checkNodeAgainstSelector(root, p.eleName[0], p.selectorAttrName, p.selectorAttrValue)){
			if (m_SelectorMaps.find(p.rawValue) == m_SelectorMaps.end()){
				std::vector<std::string> temp;
				temp.push_back(curXPath);
				m_SelectorMaps.insert(std::make_pair(p.rawValue, temp));
			}
			else{
				m_SelectorMaps[p.rawValue].push_back(curXPath);
			}
		}
	}
	//visit all children
	...
	while (next != nullptr && next != NULL){
		...
		mapSelectorToXPathVectors(next, pWithSelector, curXPath, elements[nextNodeName]);
...
std::string nsDocument::checkPolicyAndOutputToString(std::string pfRoot){
	...
	for (auto domain : mRecords){
		...
		for (auto ra_r : this->mRecords[domain.first].ra_r){
			...
			if (shouldOutput){
				...
				if (m_matchedRecords.find(domain.first) == m_matchedRecords.end()){
					records recs;
					recs.ra_r.insert(ra_r);
					std::map<std::string, nsIDocument::records> pc;
					pc.insert(std::pair<std::string, records>(pPtr.rawValue, recs));
					m_matchedRecords.insert(std::pair<std::string, std::map<std::string, nsIDocument::records>>(domain.first, pc));
				}
				else{
					if (m_matchedRecords[domain.first].find(pPtr.rawValue) == m_matchedRecords[domain.first].end()){
						records recs;
						recs.ra_r.insert(ra_r);
						m_matchedRecords[domain.first].insert(std::pair<std::string, records>(pPtr.rawValue, recs));
					}
					else{
						m_matchedRecords[domain.first][pPtr.rawValue].ra_r.insert(ra_r);
					}
				}
			}
		}
	}
	//merge m_matchedRecords with m_matchedDeletedRecords
	for (auto item : m_matchedDeletedRecords){
		if (m_matchedRecords.find(item.first) == m_matchedRecords.end()) m_matchedRecords.insert(item);
		else {
			for (auto item2 : item.second){
				if (m_matchedRecords[item.first].find(item2.first) == m_matchedRecords[item.first].end()) m_matchedRecords[item.first].insert(item2);
				else m_matchedRecords[item.first][item2.first].ra_r.insert(item2.second.ra_r.begin(), item2.second.ra_r.end());
			}
		}
	}
	//merge m_violatedRecords with m_violatedDeletedRecords
	for (auto item : m_violatedDeletedRecords){
		if (m_violatedRecords.find(item.first) == m_violatedRecords.end()) m_violatedRecords.insert(item);
		else m_violatedRecords[item.first].ra_r.insert(item.second.ra_r.begin(), item.second.ra_r.end());
	}
	std::vector<std::string> outputed;
	for (auto domain : m_matchedRecords){
		s += "tpd: " + domain.first + ":\n";
		outputed.push_back(domain.first);
		for (auto pRV : domain.second){
			s += "M: " + pRV.first + " <-with-> " + std::to_string(pRV.second.ra_r.size()) + "\n";
		}
		for (auto ra_r : m_violatedRecords[domain.first].ra_r){
			//violation of policies, output to string.
	...
	for (auto domain : m_violatedRecords){
		if (std::find(outputed.begin(), outputed.end(), domain.first) != outputed.end()) continue;
		s += "tpd: " + domain.first + ":\n";
