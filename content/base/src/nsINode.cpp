...
nsINode::original(){
	if (m_original != -1){
		return m_original;		//cached results
	}
	m_original = 1;				//default to an original node (owned by the trusted party)
	nsIDocument *d = this->OwnerDoc();
	if (d != nullptr && !d->loadedFriendDomains){
		d->loadFriendDomains();
	}
	if (node3POwners.empty()) {
		return m_original;
	}
	if (d->friendDomains.empty()) {
		//no friend domains yet this one has a creator, must not be an original node.
		m_original = 0;
		return m_original;
	}
	for (auto s : node3POwners){
		//if it is owned by any third-party, it is not an original node.  Its creator must all fall inside the friend domains for it to be an original node.
		if (d->friendDomains.find(s) == d->friendDomains.end()) {
			m_original = 0;
			break;
		}
	}
