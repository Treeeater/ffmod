...
{
  ErrorResult rv;
  GetCookie(NULL, aCookie, rv);
  return rv.ErrorCode();
}

void
nsHTMLDocument::GetCookie(JSContext *cx, nsAString& aCookie, ErrorResult& rv)
{
...
void
nsHTMLDocument::SetCookie(JSContext *cx, const nsAString& aCookie, ErrorResult& rv)
{
	if (mDisableCookieAccess) {
		return;
	}
...
/* DOMString queryCommandValue(in DOMString commandID); */
NS_IMETHODIMP
nsHTMLDocument::VisualizerOutputToStringAdd(nsAString &_retval)
{
	//give retval the nsstring value.
	this->clearDOMAccess();
	this->collectDOMAccess(this->GetBodyElement(), "", "", 1);
...
nsHTMLDocument::Write(JSContext* cx, const Sequence<nsString>& aText,
                      ErrorResult& rv)
{
  char *s = JS_EncodeString(cx, JS_ComputeStackString(cx));
