...
public:
  ...
  already_AddRefed<IDBOpenDBRequest>
	  Open(JSContext *cx, const nsAString& aName, const IDBOpenDBOptions& aOptions,
	  ErrorResult& aRv){
	  return Open(aName, aOptions, aRv);
  }
...
private:
  ...
  already_AddRefed<IDBOpenDBRequest>
	  Open(JSContext *cx, nsIPrincipal* aPrincipal, const nsAString& aName,
	  const Optional<uint64_t>& aVersion,
	  const Optional<mozilla::dom::StorageType>& aStorageType, bool aDelete,
	  ErrorResult& aRv){
	  return Open(aPrincipal, aName, aVersion, aStorageType, aDelete, aRv);
  }
